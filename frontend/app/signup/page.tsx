...
export default function SignUpPage() {
  ...
  const onSubmit = async () => {
    try {
      setLoading(true);
      validate();
      const response = await api.auth.signUp(state.name, state.password);
  ...
  return (
    <div className="tw-grid tw-grid-cols-2 tw-max-h-[100vh] tw-h-[100vh]">
      ...
      <div className="tw-flex tw-flex-col tw-h-[100%] tw-mx-10 tw-my-10">
        <h1>Welcome Back</h1>
        <h1 className="tw-italic">Lets start by creating an account!</h1>
        ...
        <p className="tw-no-underline tw-text-lg">
